...
async function test() {
  const { error: insErr } = await supabase.from('pre_revision_archivos').insert({ modalidad_id: 'test', csv_data: [] });
  console.log('insert error:', insErr);
  const { error: delErr } = await supabase.from('pre_revision_archivos').delete().eq('modalidad_id', 'test');
